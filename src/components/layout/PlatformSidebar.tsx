...
export default function PlatformSidebar({ role, onLinkClick, onClose }: PlatformSidebarProps) {
  ...
  const coachLinks: SidebarLink[] = [
    ...
    { href: '/platform/coach/squad', label: 'Squad Roster', icon: Users },
    { href: '/platform/coach/analytics', label: 'Analytics', icon: BarChart3 },
  ...
  return (
    <div className="flex flex-col h-full bg-[#111827] border-r border-zinc-800 select-none text-zinc-100">
      {/* Brand Header */}
      <div className="h-16 flex items-center px-6 border-b border-zinc-800 justify-between shrink-0">
        ...
        {links.map((link) => {
          ...
          return (
            <Link
              ...
              href={link.href}
              onClick={onLinkClick}
              className={`flex items-center space-x-3 px-4 py-2.5 rounded-md text-sm font-semibold transition-colors group ${
                active
      ...
      <div className="p-4 border-t border-zinc-800 shrink-0 bg-black/10">
        <div className="flex items-center space-x-3 px-2 py-1.5 mb-2">
          <div className="h-9 w-9 rounded-md bg-zinc-900 border border-zinc-700 flex items-center justify-center font-bold text-white select-none">
            {mockUser.initial}
          </div>
          <div className="overflow-hidden">
            <p className="text-xs font-bold text-zinc-200 truncate">{mockUser.name}</p>
            <p className="text-[9px] text-zinc-500 truncate">{mockUser.detail}</p>
          </div>
        </div>
        <Link 
          href="/login" 
          className="flex items-center space-x-3 px-4 py-2 rounded-lg text-xs font-bold text-zinc-400 hover:text-white hover:bg-zinc-900 transition-colors w-full"
        >
